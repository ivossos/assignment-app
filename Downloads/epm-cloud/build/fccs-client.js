...
export class FccsClient {
    ...
    async runMdx(application, query) {
        if (config.FCCS_MOCK_MODE) {
            if (config.FCCS_LANGUAGE === 'pt') {
                return {
    ...
    async runJob(application, jobType, jobName, parameters = {}) {
        if (config.FCCS_MOCK_MODE) {
    ...
    async getJobStatus(application, jobId) {
        if (config.FCCS_MOCK_MODE) {
            const isPt = config.FCCS_LANGUAGE === 'pt';
            const statuses = isPt
    ...
    async getDimensions(application) {
        if (config.FCCS_MOCK_MODE) {
            if (config.FCCS_LANGUAGE === 'pt') {
                return {
                    items: [
    ...
    async generateReport(application, reportName, parameters = {}) {
        if (config.FCCS_MOCK_MODE) {
            const isPt = config.FCCS_LANGUAGE === 'pt';
            return {
    ...
    async simulateTransaction(application, type, entity, percentage, amount) {
        if (config.FCCS_MOCK_MODE) {
            const isPt = config.FCCS_LANGUAGE === 'pt';
